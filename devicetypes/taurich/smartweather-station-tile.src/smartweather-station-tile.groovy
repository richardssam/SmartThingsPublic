 ...
 *  Author: Sam Richards
 *
 *  Date: 2016-07-05 -- based on the smart things one.
 */
// for the UI
metadata {
	// Automatically generated. Make future change here.
	definition (name: "SmartWeather Station Tile", namespace: "taurich", author: "Sam Richards") {
        ...
        attribute "forecastHighLow", "string"
	...
	tiles {
    	...
    	valueTile("forecastHighLow", "device.forecastHighLow", decoration: "flat"){
        	state "default", label: '${currentValue}'
         }
        ...
        main("forecastHighLow")
		details(["temperature", "weatherIcon","feelsLike", "forecastHigh", "forecastLow", "forecastHighLow", "wind","windDir","weather", "city","percentPrecip", "refresh","alert", "humidity","rise","set","light"])}
}
...
def poll() {
	...
	if (obs) {
		...
		if (f1) {
            ...
            def high = f1[0].high.fahrenheit as String 
            def low = f1[0].low.fahrenheit as String 
            send(name: "forecastHigh", value: "High $high")
            send(name: "forecastLow", value: "Low $low")
            send(name: "forecastHighLow", value: "$high / $low")
            log.debug "Got Forecast"
		}
		else {
...
private get(feature) {
	getWeatherFeature(feature, "KLAX")
}
